...
我们知道 `opacity` 属性是用来设置元素不透明度的。但可能知道 `opacity` 和层叠上下文有关的不多，不过没关系，这里我们简单聊聊这个话题。

* 当opacity值小于1时，该元素会创建新的层叠上下文，也就是说它可以和定位元素进行层叠层别比较
* 当opacity值小于1时，该元素层叠级别相当于z-index:0或auto，但不能定义 z-index ，除非本身是定位元素
...
先来看看一个图文替换的例子 `DEMO5`: [图文替换实例](http://demo.doyoe.com/css/z-index/back-top.htm)。
